...
<body>
  ...
  // if connection is error, output error message on screen
  if ($connection->connect_error != null) {
    ...
    if (!empty($_POST)) {
      ...
      $sql = "UPDATE users SET fullname = '$fullname', username = '$username', password = '$password', email = '$email' WHERE id = '$user_id'";
    ...
    $user = $result->fetch_array() ?? false; // PHP 7 has added a new operator (??)
  }
  ?>
  ...
  <?php if ($user) { ?>
    ...
    <!-- if there is no user (id is not exist), show error message -->
  <?php } else { ?>
    <p>User not found</p>
  <?php } ?>
</body>
